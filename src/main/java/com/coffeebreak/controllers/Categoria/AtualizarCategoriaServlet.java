package com.coffeebreak.controllers.Categoria;

import com.coffeebreak.models.Categoria.Categoria;
import com.coffeebreak.models.Categoria.CategoriaDAO;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;

import java.io.IOException;
import java.net.URLEncoder;
import java.nio.charset.StandardCharsets;


@WebServlet("/admin/categoria/atualizar")
public class AtualizarCategoriaServlet extends HttpServlet {

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession sessao = request.getSession(false);
        if (sessao == null || sessao.getAttribute("adminLogado") == null) {
            response.sendRedirect(request.getContextPath() + "/loginAdmin");
            return;
        }

        String idStr = request.getParameter("id");
        if (idStr == null || idStr.isBlank()) {
            redirecionar(request, response, false, "Categoria inválida.");
            return;
        }

        try {
            int id = Integer.parseInt(idStr);

            CategoriaDAO categoriaDAO = new CategoriaDAO();
            Categoria categoriaExistente = categoriaDAO.obterPeloId(id);

            if (categoriaExistente == null) {
                redirecionar(request, response, false, "Categoria não encontrada.");
                return;
            }

            String nome = request.getParameter("nome");
            String descricao = request.getParameter("descricao");

            if (nome != null && !nome.isBlank()) {
                categoriaExistente.setNome(nome.trim());
            }
            if (descricao != null && !descricao.isBlank()) {
                categoriaExistente.setDescricao(descricao.trim());
            }

            boolean atualizada = categoriaDAO.atualizarCategoria(categoriaExistente);

            if (atualizada) {
                redirecionar(request, response, true, "Categoria atualizada com sucesso!");
            } else {
                redirecionar(request, response, false, "Não foi possível atualizar a categoria.");
            }

        } catch (NumberFormatException e) {
            redirecionar(request, response, false, "Categoria inválida.");
        }
    }

    private void redirecionar(HttpServletRequest request, HttpServletResponse response,
                              boolean sucesso, String mensagem) throws IOException {
        String chave = sucesso ? "sucesso" : "erro";
        String url = request.getContextPath() + "/admin/dashboard?secao=categorias&" + chave
                + "=" + URLEncoder.encode(mensagem, StandardCharsets.UTF_8);
        response.sendRedirect(url);
    }
}
